...
model = load_model("iteration3/nervous_intensity_modelEMO.h5")
...
label_encoder.fit(nervous_df['emotion_label'])
...
def predict_emotion_intensity(audio_path, model, scaler, label_encoder):
    ...
    # Make a prediction using the loaded model (probabilities for each class)
    prediction_probabilities = model.predict(audio_features_reshaped)
    
    # Get the predicted class for intensity
    intensity_label = np.argmax(prediction_probabilities)
    ...
    predicted_probability = prediction_probabilities[0][intensity_label] * 100
    
    # Print probabilities of all classes (optional, for debugging or analysis)
    print(f"Probabilities for all classes: {prediction_probabilities}")
    
    return intensity_label_decoded, predicted_probability

# Example usage: predict the nervousness intensity for a new audio file
audio_file_path = "iteration3/emo fearfull monsta.wav"
predicted_intensity_label, predicted_percentage = predict_emotion_intensity(audio_file_path, model, scaler, label_encoder)
print(f"Predicted emotion label: {predicted_intensity_label[0]}")
print(f"Predicted emotion confidence: {predicted_percentage:.2f}%")
